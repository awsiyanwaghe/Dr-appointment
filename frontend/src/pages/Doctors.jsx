  ...
  const applyFilter = () => {
    if (speciality) {
      ...
      const filtered = doctors.filter(
        (doc) => doc.speciality.trim().toLowerCase() === normalized
      );
      setfilterdoc(filtered);
    } else {
      setfilterdoc(doctors);
    }
    setLoading(false); // ✅ doctors filter hone ke baad loader band
  };
        ...
        <div className="w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 gap-y-6 px-2">
          {filterdoc.map((itme, index) => (
            ...
            >
              ...
              <div className="p-4">
                <div
                  className={`flex items-center gap-2 text-sm text-center ${itme.available ? "text-green-500" : "text-red-500"
                    }`}
                >
                  <p
                    className={`w-2 h-2 ${itme.available ? "bg-green-500" : "bg-red-500"
                      } rounded-full`}
                  ></p>
                  <p>{itme.available ? "Available" : "Not available"}</p>
                </div>
